...
\ifio
    ...
    \hspace{12em}($\mathrm{Priv}=1$ is supervisor mode. $\mathrm{PRIO}={111}_2$ is highest.)
...
\ifdatapath
    \ifcconv
        \includegraphics[width=0.9\linewidth]{figs/lc3-memory-map-final}
    \else
        \includegraphics[width=0.33\linewidth]{figs/lc3-memory-map}
    \fi
\else
    \hspace{4em}\includegraphics[width=0.5\linewidth]{figs/lc3-memory-map-c}
\fi
